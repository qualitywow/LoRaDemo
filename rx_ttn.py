...
class LoRaWANrcv(LoRa):
    ...
    def on_rx_done(self):
        ...
        if(count):
            temp = self.gateway.A
            for i in range (self.gateway.b):
                self.gateway.A *= temp 
            k = self.gateway.A % self.gateway.p
            print("k:",k)
            for i in range(1,len(payload)-4):
                payload[i] -= k
